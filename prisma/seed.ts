...
const MANAGER_EMAIL = "manager@clementspest.com";
const MANAGER_PASSWORD = "clements123";

async function main() {
  // Wipe in dependency order so `npm run db:seed` is repeatable.
  ...
  // --- Warehouses -----------------------------------------------------
  const vero = await prisma.warehouse.create({
    data: { name: "Vero Beach (HQ)" },
  });
  ...
  const manager = await prisma.user.create({
    data: {
      ...
      role: "admin",
      warehouseId: vero.id, // default warehouse; admin can act on all
    },
  });
  ...
  const techs = [] as { id: string; homeWarehouseId: string }[];
  ...
  // --- Products (realistic FL pest-control catalog) -------------------
  const productSpec = [
  ...
  // Two SiteOne invoices into Vero Beach so purchased/on-hand and price
  // history all have data. The second invoice bumps one price to seed a
  // price-increase anomaly the alerts agent can flag later.
  ...
  async function seedInvoice(opts: {
    ...
    number: string;
    date: Date;
    lines: Array<[keyof typeof products & string, number, number]>; // [name, qty, unitPrice]
  }) {
    ...
    // Corresponding positive check_in movements.
  ...
  await seedInvoice({
    ...
    date: daysAgo(5),
    lines: [
      ["Termidor SC", 6, 92.75], // ~18% jump vs 78.50 -> price_increase later
  ...
  // --- Sample check-outs to technicians -------------------------------
  const checkout = async (
    name: keyof typeof products & string,
    warehouseId: string,
    techId: string,
    qty: number,
    dAgo: number
  ...
  console.log(`  Warehouses: 3   Technicians: ${techs.length}   Products: ${Object.keys(products).length}`);
