    ...
    <>
      ...
      {!locked ? (
        <div className="mb-4 flex flex-wrap gap-1 rounded-xl bg-black/20 p-1 w-fit">
          {BRANCHES.map((b) => (
            <Link key={b.key} href={`/my-branch/documents?branch=${b.key}`} className={`rounded-lg px-3 py-1.5 text-xs font-medium transition-colors ${branch === b.key ? "bg-emerald-grad text-white shadow" : "text-muted hover:text-ink"}`}>{b.label}</Link>
          ))}
        </div>
      ) : null}
