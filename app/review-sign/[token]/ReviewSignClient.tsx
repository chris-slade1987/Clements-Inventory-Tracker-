...
const STATEMENT =
  "I acknowledge that I participated in this review and that it was discussed with me. My signature indicates receipt and participation, not necessarily agreement.";

export default function ReviewSignClient({ token, defaultName }: { token: string; defaultName: string }) {
  ...
  async function sign() {
    ...
    const res = await fetch("/api/review-sign", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ token, signerName: name, agree }) });
  ...
  if (done) {
    return (
      <div className="mt-4 rounded-lg bg-emerald-50 border border-emerald-200 p-4 text-center">
        ...
        <p className="mt-1 text-sm text-emerald-800">Thank you, {name}. Your signature has been recorded. It now returns to HR for final approval. You may close this page.</p>
      </div>
    );
  }
  ...
  return (
    <div className="mt-4 border-t border-slate-200 pt-4">
      <div className="text-sm font-medium text-slate-900 mb-1">Sign as the employee</div>
