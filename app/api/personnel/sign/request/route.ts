...
export async function POST(req: Request) {
  ...
  const res = await sendEmail({
    ...
    html: `<p>You have a <strong>${label.toLowerCase()}</strong> to review and sign as <strong>${roleDef.label}</strong>.</p><p><a href="${link}">Review &amp; e-sign →</a></p><p>Please sign within 24 hours. You'll receive daily reminders until it's signed.</p><p>— Clements Command &amp; Control</p>`,
  });
