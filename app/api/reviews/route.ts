  ...
  try {
    ...
    if (action === "send") {
      ...
      await sendEmail({
        ...
        html: `<p>You've been asked to conduct <strong>${review.employee.name}</strong>'s <strong>${label}</strong>${b}, due ${review.dueDate.toLocaleDateString()}.</p><p><a href="${link(review.id)}">Open the review →</a></p><p>Complete it with the employee, then both of you sign. It then returns to HR for final approval.</p><p>— CanopyOS</p>`,
      });
      // Notify the employee.
      if (review.employee.email) {
        await sendEmail({
          ...
          html: `<p>Hi ${review.employee.name.split(" ")[0]},</p><p>Your <strong>${label}</strong> with ${reviewer.name} is scheduled (target ${review.dueDate.toLocaleDateString()}). You'll complete it together and sign it in the portal.</p><p>— CanopyOS</p>`,
        });
      }
    ...
    if (action === "sign") {
      ...
      if (role === "reviewer") {
        ...
        if (done.length)
          await sendEmail({
            ...
            html: `<p><strong>${review.employee.name}</strong>'s ${label} has been approved by HR and filed to the personnel record. Thank you.</p><p>— CanopyOS</p>`,
          });
      ...
      if (fresh && fresh.reviewerSignedAt && fresh.employeeSignedAt && fresh.status !== "pending_approval" && fresh.status !== "completed") {
        ...
        await sendEmail({
          ...
          html: `<p><strong>${review.employee.name}</strong>${b} and their reviewer have both signed the <strong>${label}</strong>.</p><p><a href="${link(review.id)}">Review &amp; give final approval →</a></p><p>— CanopyOS</p>`,
        });
      }
      return NextResponse.json({ ok: true });
    }
